...
bool JNetwork::InitServer(int protocol, int iport, const char* ip)
{
...
int JNetwork::SendMsg(SOCKET sock, char* msg, WORD type)
{
	// 1번 패킷 생성
	UPACKET packet;
...
int JNetwork::SendMsg(SOCKET sock, char* msg, int iSize,  WORD type)
{
	// 1번 패킷 생성
	UPACKET packet;
	ZeroMemory(&packet, sizeof(packet));
...
int JNetwork::SendMsg(SOCKET sock, UPACKET& packet)
{
	char* pMsg = (char*)&packet;
	int iSendSize = 0;
	do {
...
int JNetwork::AddUser(SOCKET sock)
{
...
int JNetwork::RecvUser(JNetUser& user)
{
