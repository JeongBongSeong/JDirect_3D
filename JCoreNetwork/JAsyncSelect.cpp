...
bool    JAsyncSelect::Connect(HWND hWnd, int protocol, int iport, const char* ip)
{
...
LRESULT  JAsyncSelect::MsgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{	
	case NETWORK_MSG:
	{
		...
		case FD_CONNECT :
		{
			...
			SendMsg(m_Sock, (char*)&login, sizeof(JLoginReq), PACKET_LOGIN_REQ);
