...
bool    JWriteFont::SetRenderTarget(IDXGISurface1* pSurface)
{
...
bool	JWriteFont::Frame()
{
	return true;
}

bool	JWriteFont::Render()
{
	m_pd2dRT->BeginDraw();
	/*D2D_RECT_F rt;
	rt.top = 0;
...
void    JWriteFont::Draw(std::wstring msg, RECT rt, D2D1::ColorF color, IDWriteTextFormat* tf)
{
	m_pd2dRT->BeginDraw();
	D2D_RECT_F fRT;
	fRT.top = rt.top;
