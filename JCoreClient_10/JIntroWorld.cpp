...
bool JIntroWorld::CreateModelType()
{
	...
	btnDlg->SetPosition(JVector2(400, 300));
	...
	pDlgWindow->SetPosition(JVector2(400, 300));
	...
	pNewDlgBtn->AddPosition(JVector2(0, 0));
...
bool JIntroWorld::Load(std::wstring filename)
{
	...
	JUIModel* pNewBK = I_UI.GetPtr(L"bk")->Clone();// new TButtonObject(*I_UI.GetPtr(L"btnStart"));
	...
	JUIModel* pNewBtn = I_UI.GetPtr(L"btnStart")->Clone();// new TButtonObject(*I_UI.GetPtr(L"btnStart"));
	pNewBtn->m_csName = L"btnStartClone1";
	pNewBtn->SetRectDraw({ 0,0, 100,50 });
	pNewBtn->AddPosition(JVector2(300, 0));
	pNewBtn->UpdateData();
	m_UIObj.push_back(pNewBtn);

	JUIModel* pNewBtn2 = I_UI.GetPtr(L"btnStart")->Clone();// new TButtonObject(*I_UI.GetPtr(L"btnStart"));
	pNewBtn2->m_csName = L"btnStartClone2";
	pNewBtn2->SetRectDraw({ 0,0, 100,50 });
	pNewBtn2->AddPosition(JVector2(300, 50));
	pNewBtn2->UpdateData();
	...
	pNewBtn->SetRectDraw({ 0,0, 100,50 });
	pNewBtn->AddPosition(JVector2(300, 100));
	pNewBtn->UpdateData();
	m_UIObj.push_back(pNewBtn);

	JUIModel* pNewDlgBtnClone = I_UI.GetPtr(L"dlgWindow")->Clone();// new TButtonObject(*I_UI.GetPtr(L"btnStart"));
