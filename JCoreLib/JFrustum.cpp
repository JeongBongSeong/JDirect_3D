...
void JFrustum::CreateFrustum(T::TMatrix& matView, T::TMatrix& matProj)
{
	m_vFrustum[0] = TVector3(-1.0f, -1.0f, 0.0f);
	m_vFrustum[1] = TVector3(-1.0f, 1.0f, 0.0f);
	m_vFrustum[2] = TVector3(1.0f, 1.0f, 0.0f);
...
BOOL JFrustum::ClassifyPoint(T::TVector3* v)
{
	for (int i = 0; i < 6; i++)
	{
		//6개의 평면의 노말을 통해서 평면의 방정식 적용
		float pToc = m_Plane[i].x * v->x +
			m_Plane[i].y * v->y +
			m_Plane[i].z * v->z +
			m_Plane[i].w;
		//값이 0보다 크면 노말에 앞쪽에 존재하는것 즉, 절두체의 바깥쪽에 존재
		if (pToc > 0.0f)
		{
			return FALSE;
		}
...
BOOL JFrustum::ClassifySphere(JSphere* v)
{
	for (int i = 0; i < 6; i++)
	{
		float pToc = m_Plane[i].x * v->vMiddle.x +
			m_Plane[i].y * v->vMiddle.y +
...
BOOL JFrustum::ClassifyOBB(JBox* box)
{
	float sum = 0.0f;
	T::TVector3 vDir;
	for (int i = 0; i < 6; i++)
